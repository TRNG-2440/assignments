...
# TODO Question 7
# Write your PySpark solution below.
drop_booking_df = bookings_df.drop("services", "payment_mode")

drop_booking_df.show()
...
# TODO Question 12
# Write your PySpark solution below.
display(bookings_df.filter(F.col("baggage_kg").between(8, 10)))
...
# TODO Question 14
# Write your PySpark solution below.
deduped_df = bookings_df.dropDuplicates(["booking_id"])
display(deduped_df)
deduped_df.count()
...
display(
    bookings_df.select(
        F.initcap(F.trim(F.col("passenger_name"))).alias("passenger_name_trim_initcap")
    )
)
...
display(
    bookings_df.select(
        F.upper(F.col("passenger_name")).alias("passenger_name_upper"),
        F.lower(F.col("passenger_name")).alias("passenger_name_lower"),
        F.initcap(F.col("passenger_name")).alias("passenger_name_title")
    )
)
...
display(
    bookings_df.select(
        F.trim(F.col("passenger_name")),
        F.length(F.trim(F.col("passenger_name"))).alias("passenger_name_length")
    )
)
...
display(
    bookings_df.select(
        F.trim(F.col("passenger_name")),
        F.substring(F.trim(F.col("passenger_name")), 1 ,5)
    )
)
...
# TODO Question 20
# Write your PySpark solution below.
summary_bookings_df = bookings_df.select(
    "*",
    F.concat_ws(" - ", F.initcap(F.trim(F.col("passenger_name"))), F.col("airline_code"), F.col("route_code"), F.col("seat_class")).alias("booking_summary")
)

display(summary_bookings_df)
...
display(
    bookings_df.select(
        F.regexp_replace(F.col("passenger_name"), r"\s+", " ").alias("clean_name")
    )
)
...
display(
    bookings_df.select(
        F.split(F.col("route_code"), "-").alias("parts")
    )
)
...
display(
    bookings_df.select(
        ...
        F.col("origin_city").contains("an") |
        F.col("origin_city").startswith("B") |
        F.col("origin_city").endswith("i")
    )
)
...
display(
    bookings_df.select(
        F.round(F.col("ticket_amount") / 3, 2).alias("rounded_amt"),
        F.ceil(F.col("ticket_amount") / 3).alias("ceil_amt"),
        F.floor(F.col("ticket_amount") / 3).alias("floor_amt")
    )
)
...
display(
    bookings_df.select(
        F.abs(F.col("ticket_amount") / 7000).alias("abs_amt")
    )
)
...
display(
    bookings_df.select(
        F.sqrt(F.col("ticket_amount")).alias("sqrt_amt")
    )
)
...
display(
    bookings_df.select(
        (F.col("ticket_amount") * F.lit(1.08)).alias("fare_plus_tax"),
        (F.col("ticket_amount") - F.coalesce(F.col("promo_discount"), F.lit(0))).alias("fare_after_flat_discount"),
        (F.col("baggage_kg") * 2).alias("doubled_baggage"),
        (
            F.col("ticket_amount") /
            F.when(F.col("baggage_kg") == 0, F.lit(1)).otherwise(F.col("baggage_kg"))
        ).alias("fare_per_kg")
    )
)
...
# TODO Question 32
# Write your PySpark solution below.
display(bookings_df.dropna(subset=["satisfaction_score"]))
...
display(
    bookings_df.select(
        F.coalesce(F.col("promo_discount"), F.lit(0)).alias("final_discount")
    )
)
...
display(
    bookings_df.select(
        F.col("booking_id").cast("string").alias("booking_id_str"),
        F.col("satisfaction_score").cast("int").alias("satisfaction_score_int"),
        F.col("ticket_amount").cast("decimal(10,2)").alias("ticket_amount_decimal")
    )
)
print(bookings_df.select(
    F.col("booking_id").cast("string"),
    F.col("satisfaction_score").cast("int"),
...
display(
    bookings_df.select(
        F.to_date(F.col("booking_date"), "yyyy-MM-dd")
    )
)
...
display(
    bookings_df.select(
        F.year(F.col("travel_date")).alias("year"),
        F.month(F.col("travel_date")).alias("month"),
        F.dayofmonth(F.col("travel_date")).alias("day"),
        F.dayofweek(F.col("travel_date")).alias("weekday")
    )
)
...
display(
    bookings_df.select(
        F.date_format(F.col("travel_date"), "dd-MMM-yyyy").alias("day_month_year")
    )
)
...
display(
    bookings_df.select(
        F.datediff(F.current_date(), F.col("booking_date")).alias("days_since_booking"),
...
# TODO Question 41
# Write your PySpark solution below.
aggregate_airlines_df = bookings_df.select(
        F.count(F.col("booking_id")).alias("booking_count"),
        F.sum(F.col("ticket_amount")).alias("total_fare"),
        F.avg(F.col("ticket_amount")).alias("avg_fare"),
        F.max(F.col("ticket_amount")).alias("max_fare"),
        F.min(F.col("ticket_amount")).alias("min_fare"),
        F.countDistinct(F.col("airline_code")).alias("airline_count")
    )

display(aggregate_airlines_df)
...
# Write your PySpark solution below.
display(
    bookings_df.groupBy(F.col("airline_code")).agg(
        F.count(F.col("booking_id")).alias("booking_count"),
        F.avg(F.col("ticket_amount")).alias("avg_fare"),
        F.sum(F.col("ticket_amount")).alias("total_fare"),
        F.min(F.col("ticket_amount")).alias("min_fare"),
...
# Write your PySpark solution below.
display(
    bookings_df.orderBy(F.col("ticket_amount").asc())
)

display(
    bookings_df.orderBy(F.col("ticket_amount").desc())
)
...
# TODO Question 48
# Write your PySpark solution below.
left_join_df = bookings_df.join(
    airline_df,
    on="airline_code",
    how="left"
).where(
    F.col("headquarters").isNull()
)
...
# TODO Question 49
# Write your PySpark solution below.
right_join_df = bookings_df.join(
    airline_df,
    on="airline_code",
    how="right"
).where(
...
# TODO Question 53
# Write your PySpark solution below.
seat_class_df = spark.createDataFrame(
    [("Economy",), ("Business",), ("First",)],
    ["seat_class"]
)

airline_code_df = airline_df.select("airline_code").distinct()

cross_df = seat_class_df.crossJoin(airline_code_df)
...
# TODO Question 54
# Write your PySpark solution below.
union_df = bookings_df.unionByName(new_bookings_df)
...
# Write your PySpark solution below.
q57_sql = spark.sql("""
SELECT INITCAP(TRIM(passenger_name)), (LENGTH(passenger_name) - 1)
FROM bookings           
""")
...
SELECT
  *,
  CASE
    ...
    WHEN ticket_amount >= 7000 THEN 'Standard'
    ELSE 'Budget'
...
# Write your PySpark solution below.
q59_sql = spark.sql("""
SELECT *, COALESCE(promo_discount, 0) AS clean_promo_discount
FROM bookings
""")
...
SELECT
  ...
  MIN(ticket_amount) AS min_ticket_amount,
  MAX(ticket_amount) AS max_ticket_amount,
  COUNT(DISTINCT passenger_name) AS distinct_passengers
FROM bookings
...
# TODO Question 63
# Write your PySpark solution below.
q60_sql.write.mode("overwrite").saveAsTable("airline_aggregation")

airline_agg_tbl_df = spark.table("airline_aggregation")

display(airline_agg_tbl_df)
...
# TODO Question 64
# Write your PySpark solution below.
q60_sql.explain(mode="formatted")
...
# Transformation: group by airline and aggregate
airline_agg_df = bookings_df.groupBy("airline_code").agg(
    F.count("*").alias("booking_count"),
    F.sum("ticket_amount").alias("total_fare")
)

# Action: count rows in the aggregate
agg_row_count = airline_agg_df.count()

# Action: collect all rows from the aggregate
agg_rows = airline_agg_df.collect()

print("Number of airline rows:", agg_row_count)
for row in agg_rows:
    print(row)

display(airline_agg_df)
...
# TODO Question 66
# Write your PySpark solution below.
small_df = airline_agg_df.coalesce(1)
