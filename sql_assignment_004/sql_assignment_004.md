# Exercise Referenced Removal

Given the following tables, provide an SQL Statement that will remove all books written by Mark Twain. (*Bonus: You can do this manually by referencing the exact ID of Mark Twain in the Author table, but as a bonus, try to perform this in a way that would always work, even if we change Mark Twain's author_id [hint: subqueries])
