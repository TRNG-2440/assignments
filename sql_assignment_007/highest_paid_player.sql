...
/*
Given the following tables, provide an SQL statement that will find which 
sport has the highest paid player.
*/


-- output the sport name
-- list all the players by their salaries

-- connect team_id --> teams
    -- connect sport_id --> sport
    -- get the name of the sport corresponding that sport_id

select sport_name
from sports.sport
where sport_id = (
    select sport_id from sports.teams
    where team_id = (
        select team_id
        from sports.players
