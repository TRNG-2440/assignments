...
def create_spark() -> SparkSession:
    spark = (
        SparkSession.builder.appName("HealthcarePatientVisitRDD")
        .master("local[4]")
        .config("spark.driver.host", "127.0.0.1")
        .config("spark.driver.bindAddress", "127.0.0.1")
        .config("spark.ui.showConsoleProgress", "false")
        .config(
            "spark.local.dir", "C:/spark-temp" if os.name == "nt" else "/tmp/spark-temp"
        )
        .config(
            "spark.sql.warehouse.dir",
            "C:/spark-warehouse" if os.name == "nt" else "/tmp/spark-warehouse",
        )
        .getOrCreate()
    )
...
def main() -> None:
    ...
    try:
        ...
        print("\nDistinct cities:")
        print(
            enriched_rdd.map(lambda r: r["city"])
            .distinct()
            .sortBy(lambda x: x)
            .collect()
        )
        ...
        print(
            enriched_rdd.map(lambda r: r["department"])
            .distinct()
            .sortBy(lambda x: x)
            .collect()
        )
        ...
        print("\nRevenue by city:")
        print(
            paid_rdd.map(lambda r: (r["city"], r["final_amount"]))
        ...
        print(
            paid_rdd.map(lambda r: (r["department"], r["final_amount"]))
            .reduceByKey(lambda a, b: a + b)
            .sortBy(lambda pair: pair[1], ascending=False)
            .collect()
        )
        ...
        print(
            enriched_rdd.map(lambda r: (r["department"], r["visit_id"]))
            .groupByKey()
        ...
        opd_patients = enriched_rdd.filter(lambda r: r["visit_type"] == "OPD").map(
            lambda r: r["patient_id"]
        )
        emergency_patients = enriched_rdd.filter(
            lambda r: r["visit_type"] == "Emergency"
        ).map(lambda r: r["patient_id"])
        print("\nUnique OPD or Emergency patient IDs using union:")
        print(
            opd_patients.union(emergency_patients)
            .distinct()
            .sortBy(lambda x: x)
            .collect()
        )
