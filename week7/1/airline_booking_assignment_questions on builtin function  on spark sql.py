...
if not IS_DATABRICKS:
    ...
    spark: SparkSession =  (SparkSession.builder.appName("Hospital Medallion Pipeline")
                            .master("local[*]")
                            .getOrCreate())
...
bookings_df: DataFrame = spark.createDataFrame(bookings, columns)
airline_df: DataFrame = spark.createDataFrame(airlines, airline_columns)
route_df: DataFrame = spark.createDataFrame(routes, route_columns)
new_bookings_df: DataFrame = spark.createDataFrame(new_bookings, columns)
...
bookings_df.count()
print("Total records:", bookings_df.count())
...
display(bookings_df.select("booking_id", "passenger_name", "airline_code", "route_code", "ticket_amount"))
...
display(bookings_df.select(F.col("passenger_name"), F.col("ticket_amount")))
...
bookings_df = bookings_df.withColumn("gross_amount", F.col("ticket_amount") + F.coalesce(F.col("promo_discount"), F.lit(0)))
display(bookings_df)
...
no_services = bookings_df.drop("services", "payment_mode")
display(no_services)
...
display(bookings_df.filter(F.col("ticket_amount") > 8000))
...
display(bookings_df.where(F.col("ticket_amount") > 8000))
...
display(bookings_df.where(F.col("airline_code").isin("SKY", "NVA", "ORB")))
...
display(bookings_df.where(F.col("baggage_kg").between(10, 18)))
# COMMAND ----------
# MAGIC %md
...
display(bookings_df.select("seat_class").distinct())
...
bookings_df = bookings_df.withColumn("passenger_name", F.initcap(F.trim(F.col("passenger_name"))))
display(bookings_df)
...
bookings_df = bookings_df.withColumn("passenger_name_upper", F.upper(F.col("passenger_name"))) \
                         .withColumn("passenger_name_lower", F.lower(F.col("passenger_name"))) \
                         .withColumn("passenger_name_title", F.initcap(F.col("passenger_name")))
display(bookings_df)
...
bookings_df = bookings_df.withColumn("passenger_name_length", F.length(F.col("passenger_name")))
display(bookings_df)
...
bookings_df = bookings_df.withColumn("passenger_name_first5", F.substring(F.col("passenger_name"), 1, 5))
display(bookings_df)
...
bookings_df = bookings_df.withColumn("booking_code", F.concat(F.lit("BOOK-"), F.col("booking_id")))
display(bookings_df)
...
bookings_df = bookings_df.withColumn("booking_summary", F.concat_ws(" - ", F.col("passenger_name"), F.col("airline_code"), F.col("route_code"), F.col("seat_class")))
display(bookings_df)
...
bookings_df = bookings_df.withColumn("passenger_name", F.regexp_replace(F.col("passenger_name"), r"\s+", " "))
display(bookings_df)
...
# TODO Question 22
# Write your PySpark solution below.
bookings_df = bookings_df.withColumn("route_code", F.split(F.col("route_code"), "-"))
display(bookings_df)
...
names_with_an = bookings_df.filter(F.col("passenger_name").contains("an"))
origin_cities_starting_with_B = bookings_df.filter(F.col("origin_city").startswith("B"))
destination_cities_ending_with_i = bookings_df.filter(F.col("destination_city").endswith("i"))

display(names_with_an)
display(origin_cities_starting_with_B)
display(destination_cities_ending_with_i)
...
bookings_df = bookings_df.withColumn("ticket_amount_div_3", F.col("ticket_amount") / 3)
bookings_df = bookings_df.withColumn("ticket_amount_div_3_rounded", F.round(F.col("ticket_amount_div_3"), 2))
bookings_df = bookings_df.withColumn("ticket_amount_div_3_ceil", F.ceil(F.col("ticket_amount_div_3")))
bookings_df = bookings_df.withColumn("ticket_amount_div_3_floor", F.floor(F.col("ticket_amount_div_3")))
display(bookings_df)
...
bookings_df = bookings_df.withColumn("abs_diff_ticket_amount", F.abs(F.col("ticket_amount") - 7000))
display(bookings_df)
...
bookings_df = bookings_df.withColumn("baggage_kg_squared", F.pow(F.col("baggage_kg"), 2))
display(bookings_df)
...
bookings_df = bookings_df.withColumn("ticket_amount_sqrt", F.sqrt(F.col("ticket_amount")))
display(bookings_df)
...
bookings_df = bookings_df.withColumns(
    {
        "fare_after_flat_discount": F.col("ticket_amount") - F.col("promo_discount"),
        "doubled_baggage": F.col("baggage_kg") * 2,
        "fare_per_kg": F.when(F.col("baggage_kg") != 0, F.col("ticket_amount") / F.col("baggage_kg")).otherwise(0)
    }
)
...
display(bookings_df.filter(F.col("promo_discount").isNull()))
display(bookings_df.filter(F.col("promo_discount").isNotNull()))
...
bookings_df = bookings_df.fillna({"promo_discount": 0})
display(bookings_df)
...
bookings_df = bookings_df.dropna(subset=["satisfaction_score"])
display(bookings_df)
...
bookings_df = bookings_df.withColumn("booking_id", F.col("booking_id").cast("string")) \
...
bookings_df = bookings_df.withColumn("booking_date", F.to_date(F.col("booking_date"), "yyyy-MM-dd")) \
...
bookings_df = bookings_df.withColumn("current_date", F.current_date()) \
...
bookings_df = bookings_df.withColumn("travel_year", F.year(F.col("travel_date"))) \
                         .withColumn("travel_month", F.month(F.col("travel_date"))) \
                         .withColumn("travel_day_of_month", F.dayofmonth(F.col("travel_date"))) \
                         .withColumn("travel_day_of_week", F.dayofweek(F.col("travel_date")))
display(bookings_df)
# COMMAND ----------
# MAGIC %md
...
bookings_df = bookings_df.withColumn("travel_date_formatted", F.date_format(F.col("travel_date"), "dd-MMM-yyyy"))
display(bookings_df)
...
bookings_df = bookings_df.withColumn("booking_lead_time_days", F.datediff(F.col("travel_date"), F.col("booking_date"))) \
                         .withColumn("months_elapsed", F.months_between(F.current_date(), F.col("booking_date")))
display(bookings_df)
...
bookings_df = bookings_df.withColumn("travel_plus_2_months", F.add_months(F.col("travel_date"), 2)) \
                         .withColumn("travel_plus_7_days", F.date_add(F.col("travel_date"), 7)) \
                         .withColumn("travel_minus_3_days", F.date_sub(F.col("travel_date"), 3))
display(bookings_df)
...
seat_class_report_df = bookings_df.groupBy("seat_class").agg(
    F.collect_list("booking_status").alias("all_booking_statuses"),
    F.collect_set("payment_mode").alias("unique_payment_modes")
)
display(seat_class_report_df)
...
bookings_sorted_asc_df = bookings_df.orderBy(F.col("ticket_amount").asc())
display(bookings_sorted_asc_df)

bookings_sorted_desc_df = bookings_df.orderBy(F.col("ticket_amount").desc())
display(bookings_sorted_desc_df)
...
bookings_sorted_multi_df = bookings_df.orderBy(F.col("airline_code").asc(), F.col("ticket_amount").desc())
display(bookings_sorted_multi_df)
...
top_five_expensive_bookings_df = bookings_df.orderBy(F.col("ticket_amount").desc()).limit(5)
display(top_five_expensive_bookings_df)
...
inner_join_df = bookings_df.join(airline_df, on="airline_code", how="inner")
...
left_join_df = bookings_df.join(airline_df, on="airline_code", how="left")
...
right_join_df = bookings_df.join(airline_df, on="airline_code", how="right")
...
full_join_df = bookings_df.join(airline_df, on="airline_code", how="full")
...
left_semi_join_df = bookings_df.join(airline_df, on="airline_code", how="left_semi")
...
left_anti_join_df = bookings_df.join(airline_df, on="airline_code", how="left_anti")
...
seat_class_df = spark.createDataFrame([("Economy",), ("Business",), ("First",)], ["seat_class"])
airline_codes_df = airline_df.select("airline_code").distinct()
cross_join_df = airline_codes_df.crossJoin(seat_class_df)
...
fare_category_df = spark.sql("""
    SELECT *,
           CASE
               WHEN fare < 5000 THEN 'Low'
               WHEN fare BETWEEN 5000 AND 10000 THEN 'Medium'
               ELSE 'High'
           END AS fare_category
    FROM bookings
""")
display(fare_category_df)
...
promo_discount_df = spark.sql("""
    SELECT *,
           COALESCE(promo_discount, 0) AS promo_discount
    FROM bookings
""")
display(promo_discount_df)
...
sql_booking_route_join = spark.sql("""
    SELECT b.*, a.airline_name, r.route_name
    FROM bookings b
    JOIN airlines a ON b.airline_code = a.airline_code
    JOIN routes r ON b.route_id = r.route_id
""")
display(sql_booking_route_join)
...
spark.table("sql_airline_aggregation_df").write.saveAsTable("managed_airline_aggregation")
...
transformed_df = sql_airline_aggregation_df.withColumn("total_fare_plus_one", sql_airline_aggregation_df["total_fare"] + 1)
...
coalesced_df = sql_airline_aggregation_df.coalesce(1)
